...
MOBI_RET mobi_decompress_lz77(unsigned char *out, const unsigned char *in, size_t *len_out, const size_t len_in) {
    ...
    while (buf_in->offset < 100) {
        uint8_t byte = buffer_get8(buf_in);
        printf("standard output:%d offset:%d\n",byte,buf_in->offset);
        ...
        /* 0x8000 + (distance << 3) + ((length-3) & 0x07) */
            ...
            printf("distance output:%d %d %d %d\n",byte,next,distance,length);
            while (length--) {
...
static MOBI_RET mobi_decompress_huffman_internal(MOBIBuffer *buf_out, MOBIBuffer *buf_in, const MOBIHuffCdic *huffcdic, size_t depth) {
    ...
    while (ret == MOBI_SUCCESS) {
        ...
        //printf("buffer %-25ld, code: %-15d,coderl 24bit:%-10d, t1:%-10d, maxcode: %-15d,index: %-10d ,cdicIndex:%-10d,offset %-10d,symbolLength:%-10u,data1:%-10d,data2:%-10d\n",buffer,code,code>>24,t1,maxcode,index,cdic_index,offset,symbol_length,huffcdic->symbols[cdic_index][offset] , huffcdic->symbols[cdic_index][offset + 1]);

...
MOBI_RET mobi_parse_cdic(MOBIHuffCdic *huffcdic, const MOBIPdbRecord *record, const size_t num) {
    ...
    printf("index count 2 %d %d\n", index_count >> code_length);
...
MOBI_RET mobi_parse_huffdic(const MOBIData *m, MOBIHuffCdic *huffcdic) {
    ...
    printf("%s\n","TABL1:");
    for(int i=0;i<256;i++){
        printf("%d ", huffcdic->table1[i]);
    }
    printf("\n");
    printf("%s\n", "MINCODE:");
    for (int i = 0; i < 33; ++i)
    {
        printf("%d ", huffcdic->mincode_table[i]);
    }
    printf("\n");
    printf("%s\n", "MAXCODE:");
    for (int i = 0; i < 33; ++i)
    {
        printf("%d ", huffcdic->maxcode_table[i]);
    }
    printf("\n");
...
MOBI_RET mobi_load_file(MOBIData *m, FILE *file) {
    ...
    if (m->eh) {
        ...
        printf("调试 boundary_rec_number:%d\n", boundary_rec_number);
